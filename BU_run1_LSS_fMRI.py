...
LSS_OUTPUT_DIR = os.path.join(IMG_DIR, "LSS/lss_run01")
...
def process_subject_lss(sub_id, cleaned_dir, output_root):
    ...
    # events_df.loc[events_df["trial_type"] == "event1", "modulation"] = events_df["Estim1"]
    events_df=events_df[["onset", "duration", "trial_type"]]
    confound_df  = confound_data[CONFOUND_COLS].fillna(0)

    TARGET_CONDITION = "feedback"
    ...
    for trial_number, i_trial in enumerate(feedback_indices, start=1):
        ...
        dm_img_path  = os.path.join(output_root,
            f"sub-{sub_id}_design_matrix_run-01_trial-{trial_name}.svg")
        ...
        beta_map_path = os.path.join(output_root,
            f"sub-{sub_id}_task-belief_run-01_trial-{trial_name}_beta_map.nii")
        ...
        plotting.plot_stat_map(
            ...
            output_file=os.path.join(output_root, f"sub-{sub_id}_zmap_run-01_trial-{trial_name}.svg"),
        )
    fourd_img = image.concat_imgs(lss_beta_maps)
    out_path  = os.path.join(output_root,
        f"sub-{sub_id}_task-belief_run-01_condition_{TARGET_CONDITION}_lss.nii")
...
def main():
    print("**** Starting LSS first-level analysis for Study 2, Run 1 ****")
