...
FIRST_LEVEL_DIR = os.path.join(IMG_DIR, "contrast/BU_run1_level1")
if os.path.exists(FIRST_LEVEL_DIR):
    shutil.rmtree(FIRST_LEVEL_DIR)
os.makedirs(FIRST_LEVEL_DIR, exist_ok=True)

CONTRASTS = [
    ("feedback_desirable", "feedback_desirable"),
    ("feedback_undesirable", "feedback_undesirable"),
    ("confi1", "confi1"),
    ("feedback_desirable-undesirable", "feedback_desirable - feedback_undesirable"),
]
...
def create_design_matrix(func_file: str, beh_file: str, confound_file: str):
    ...
    events_df = beh_data[["onset", "duration", "trial_type_contrast", "EE", "Confi1", "Estim1"]].copy()
